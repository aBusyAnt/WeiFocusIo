...
TextKit是iOS7引入的新的Text排版，原来上层的控件基于TextCore，Apple为了方便开发者，在iOS7中重写了上层控件，为开发者提供了TextKit以方便进行富文本排版，TextKit是基于TextCore的，无认是从兼容性还是为了更好的学习TextKit，我们都应该先来了解一下CoreText。
<!--more-->
...
4. 图片  
CoreText本身是提供UIImage的绘制，所以UIImage肯定只能通过Core Graphics绘制，而CoreText绘制的最小单元CTRun提供了CTRunDelegate，也就是当设置了kCTRunDelegateAttributeName过后，CTRun的绘制时所需的参考将从委托中获取，我们通过此方法可以实现图片的绘制。在需要绘制图片的位置，提前预留空白占位。
